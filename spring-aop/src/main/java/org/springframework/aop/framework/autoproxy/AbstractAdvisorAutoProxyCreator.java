...
/**
 * todo-zk 这个类主要提供了寻找Spring内部Advisor的功能；
 * 			以及为指定bean找出匹配的Advisor，并进行一些处理
...
public abstract class AbstractAdvisorAutoProxyCreator extends AbstractAutoProxyCreator {
	...
	protected Object[] getAdvicesAndAdvisorsForBean(
		...
		// todo-zk 为bean找出匹配的Advisor，算法如下：
		// 			1. 先找出所有的Advisor
		//          2. 再从advisors中找到匹配bean的Advisor
		//			3. 扩展Advisors（添加一些Advisor运行时辅助类）
		//			4. 对Advisor进行排序，这样才能按照正常的顺序执行
	...
	protected List<Advisor> findEligibleAdvisors(Class<?> beanClass, String beanName) {
		...
		// todo-zk 查找所有可以应用到当前bean的Advisor（应该进行了匹配）
	...
	protected List<Advisor> findCandidateAdvisors() {
		...
		// todo-zk 从beanFactory中找到所有Advisor类型的bean，不需要缓存，这个是Spring内部的Advisor，数量较少
	...
	protected List<Advisor> findAdvisorsThatCanApply(
		...
		// 将beanName添加到ThreadLocal中，用于Advisor匹配
		ProxyCreationContext.setCurrentProxiedBeanName(beanName);
		try {
			// todo-zk 匹配Advisor和beanClass，只有匹配的Advisor才会被返回
	...
	private class BeanFactoryAdvisorRetrievalHelperAdapter extends BeanFactoryAdvisorRetrievalHelper {
		...
		protected boolean isEligibleBean(String beanName) {
			// todo-zk 调用外部类的isEligibleAdvisorBean方法
