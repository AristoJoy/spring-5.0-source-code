...
/**
 * todo-zk 这个类主要是按照AspectJ中Advice执行顺序规则，对Advisor进行排序；为Advice添加
 * 			ExposeInvocationInterceptor辅助类；判断一个bean是否该跳过代理（AspectJPointcutAdvisor类型跳过）
...
public class AspectJAwareAdvisorAutoProxyCreator extends AbstractAdvisorAutoProxyCreator {
	...
	protected void extendAdvisors(List<Advisor> candidateAdvisors) {
		// todo-zk 为AspectJ Advice添加ExposeInvocationInterceptor，因为会在Advice会在执行过程中需要
	...
	protected boolean shouldSkip(Class<?> beanClass, String beanName) {
		...
		// todo-zk 查找所有的advisor，如果当前bean是AspectJPointcutAdvisor类型，就跳过代理
		List<Advisor> candidateAdvisors = findCandidateAdvisors();
		// 如果当前bean是AspectJPointcutAdvisor类型的，就跳过代理
