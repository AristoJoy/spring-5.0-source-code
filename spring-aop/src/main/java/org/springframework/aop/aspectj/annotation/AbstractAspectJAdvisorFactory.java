...
public abstract class AbstractAspectJAdvisorFactory implements AspectJAdvisorFactory {
	...
	public void validate(Class<?> aspectClass) throws AopConfigException {
		...
		// 去除非Aspect，perflow或percflowbelow类型的Aspect
		if (!ajType.isAspect()) {
	...
	protected static AspectJAnnotation<?> findAspectJAnnotationOnMethod(Method method) {
		// todo-zk 按照Pointcut > Around > Before > After > AfterReturning > AfterThrowing 的顺序找注解，如果找到就直接返回
	...
	protected static class AspectJAnnotation<A extends Annotation> {
		...
		private String resolveExpression(A annotation) {
			// todo-zk 按照Pointcut > Around > Before > After > AfterReturning > AfterThrowing 的顺序pointcut表达式，如果存在就直接返回
