...
class ConfigurationClassEnhancer {
	...
	private static class BeanFactoryAwareMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object obj, Method method, Object[] args, MethodProxy proxy) throws Throwable {
			// 设置beanFactory到CGLib添加的字段$$beanFactory中
			Field field = ReflectionUtils.findField(obj.getClass(), BEAN_FACTORY_FIELD);
			Assert.state(field != null, "Unable to find generated BeanFactory field");
			field.set(obj, args[0]);

			// 如果目标类实现了BeanFactoryAware即可，就调用BeanFactoryAware的实现，设置beanFactory到目标类原有的字段中
	...
	private static class BeanMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object enhancedConfigInstance, Method beanMethod, Object[] beanMethodArgs,
			...
			//一个非常牛逼的判断
			//判断到底是new 还是get
			//判断执行的方法和调用方法是不是同一个方法(根据当前调用的方法和beanMethod的名字和参数类型是否一致判断)
			if (isCurrentlyInvokedFactoryMethod(beanMethod)) {
				...
				// 别的方法调用bean方法，通过代理来调用
				return cglibMethodProxy.invokeSuper(enhancedConfigInstance, beanMethodArgs);
			}
			// 调用bean方法，非其他方法内调用，生成bean实例
		...
		private Object resolveBeanReference(Method beanMethod, Object[] beanMethodArgs,
			...
			//判断他是否正在创建(在某些自动注入场景，bean可能被标注为正在创建，为了避免异常，将beanFactory的正在创建标志设为false)
		...
		// 拦截除Object方法和setBeanFactory之外的方法
		@Override
		public boolean isMatch(Method candidateMethod) {
		...
		// 根据代理类的$$beanFactory获取beanFactory
