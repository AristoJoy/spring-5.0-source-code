...
//		Assert.notNull(environment, "Environment must not be null");
	...
	<T> void doRegisterBean(Class<T> annotatedClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		 ...
		 * 如果加了，则根据指定的Condition实现判断，是否要跳过注册
		 */
		if (this.conditionEvaluator.shouldSkip(abd.getMetadata())) {
			return;
		}
		//instanceSupplier用于生成一个bean的实例，这里设置到bd中
		...
		/**
		 * 生成类的名字通过beanNameGenerator生成，默认的会使用AnnotationBeanNameGenerator，如果在注解上定义了，则使用定义的
		 */
		String beanName = (name != null ? name : this.beanNameGenerator.generateBeanName(abd, this.registry));
		/**
		 * 处理类当中的通用注解
		 * 分析源码可以知道他主要处理
		 * Lazy、DependsOn、Primary、Role和Description注解
		 * 处理完成之后，添加到bd中
		 *
		 */
		...
		if (qualifiers != null) {
			for (Class<? extends Annotation> qualifier : qualifiers) {
				...
				//懒加载，前面加过
					...
					//TODO 如果使用了除@Primary和@Lazy以外的其他注解，则为该Bean添加一个根据名字自动装配的限定符，后面会详细介绍
					//这里难以理解
		...
		/**
		 * 这个BeanDefinitionHolder，主要绑定bd和name，简化传参
		 */
		BeanDefinitionHolder definitionHolder = new BeanDefinitionHolder(abd, beanName);

		/**
		 * TODO ScopedProxyMode 这个知识点比较复杂，需要结合web去理解
