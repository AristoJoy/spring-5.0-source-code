...
class ConfigurationClassBeanDefinitionReader {
	...
	private void loadBeanDefinitionsForBeanMethod(BeanMethod beanMethod) {
		...
		// BeanMethod是否是静态的，处理不同
		/**
		 * 调用factoryMethod的创建bean的地方：
		...
		if (metadata.isStatic()) {
			// 将其设置BeanMethod的name设置到bd的factoryMethod中，即工厂方法
			// static @Bean method
			...
			// 将其设置BeanMethod的name设置到bd的factoryMethod中，即工厂方法；
			// 此外还设置了bd的isFactoryMethodUnique为true，表示factoryMethod只会调用一次
		...
		if (proxyMode != ScopedProxyMode.NO) {	//生成新的bd替换旧的bd（应该在内部生成代理类，并将bd的类修改为代理类）
